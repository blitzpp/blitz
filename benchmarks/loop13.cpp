...
extern "C" {
  void loop13_f77(const int& N, double* x, double* a, double* b, double* c, double* d, double* y, const double& u);

  void loop13_f77overhead(const int& N, double* x, double* a, double* b, double* c, double* d, double* y, const double& u);

  void loop13_f90(const int& N, double* x, double* a, double* b, double* c, double* d, double* y, const double& u);

  void loop13_f90overhead(const int& N, double* x, double* a, double* b, double* c, double* d, double* y, const double& u);
...
void F77Version(BenchmarkExt<int>& bench, double u)
{
    ...
    while (!bench.doneImplementationBenchmark())
    {
        ...
        for (int iter=0; iter < iters; ++iter)
            loop13_f77(N, x, a, b, c, d, y, u);
        bench.stop();

        bench.startOverhead();
        for (int iter=0; iter < iters; ++iter)
            loop13_f77overhead(N, x, a, b, c, d, y, u);
...
void F90Version(BenchmarkExt<int>& bench, double u)
{
    ...
    while (!bench.doneImplementationBenchmark())
    {
        ...
        bench.start();
        for (int iter=0; iter < iters; ++iter)
            loop13_f90(N, x, a, b, c, d, y, u);
        bench.stop();

        bench.startOverhead();
        for (int iter=0; iter < iters; ++iter)
            loop13_f90overhead(N, x, a, b, c, d, y, u);
