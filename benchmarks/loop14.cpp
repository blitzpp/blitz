...
// loop14 generated by makeloops.py Tue Jun 28 15:54:20 2011
...
extern "C" {
  void loop14_f77(const int& N, double* y, double* x, double* a, double* b);
  void loop14_f77overhead(const int& N, double* y, double* x, double* a, double* b);
  void loop14_f90(const int& N, double* y, double* x, double* a, double* b);
  void loop14_f90overhead(const int& N, double* y, double* x, double* a, double* b);
  ...
  void ArrayVersion_index(BenchmarkExt<int>& bench)
{
    ...
    while (!bench.doneImplementationBenchmark())
    {
        ...
        for (long i=0; i < iters; ++i)
        {
            x = a(tensor::i)+b(tensor::i); y(tensor::i) = a(tensor::i)-b(tensor::i);
            sink();
        }
        bench.stop();
...
void F77Version(BenchmarkExt<int>& bench)
{
    ...
    while (!bench.doneImplementationBenchmark())
    {
        ...
        for (int iter=0; iter < iters; ++iter)
            loop14_f77(N, y, x, a, b);
        bench.stop();

        bench.startOverhead();
        for (int iter=0; iter < iters; ++iter)
            loop14_f77overhead(N, y, x, a, b);
...
void F90Version(BenchmarkExt<int>& bench)
{
    ...
    while (!bench.doneImplementationBenchmark())
    {
        ...
        bench.start();
        for (int iter=0; iter < iters; ++iter)
            loop14_f90(N, y, x, a, b);
        bench.stop();

        bench.startOverhead();
        for (int iter=0; iter < iters; ++iter)
            loop14_f90overhead(N, y, x, a, b);
