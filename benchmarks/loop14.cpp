...
BZ_USING_NAMESPACE(blitz)
 ...
 #define loop14_f77 loop14_f77_
 #define loop14_f77overhead loop14_f77overhead_
 #define loop14_f90 loop14_f90_
 #define loop14_f90overhead loop14_f90overhead_
#endif

#ifdef BZ_FORTRAN_SYMBOLS_WITH_DOUBLE_TRAILING_UNDERSCORES
 #define loop14_f77 loop14_f77__
 #define loop14_f77overhead loop14_f77overhead__
 #define loop14_f90 loop14_f90__
 #define loop14_f90overhead loop14_f90overhead__
#endif

#ifdef BZ_FORTRAN_SYMBOLS_CAPS
 #define loop14_f77 LOOP14_F77
 #define loop14_f77overhead LOOP14_F77OVERHEAD
 #define loop14_f90 LOOP14_F90
 #define loop14_f90overhead LOOP14_F90OVERHEAD
#endif

extern "C" {
  void loop14_f77(const int& N, double* x, double* a, double* b, double* y);

  void loop14_f77overhead(const int& N, double* x, double* a, double* b, double* y);

  void loop14_f90(const int& N, double* x, double* a, double* b, double* y);

  void loop14_f90overhead(const int& N, double* x, double* a, double* b, double* y);
...
int main()
{
    ...
    BenchmarkExt<int> bench("loop14: $x=$a+$b;$y=$a-$b", numBenchmarks);
    ...
    bench.saveMatlabGraph("loop14.m");
...
void F77Version(BenchmarkExt<int>& bench)
{
    ...
    while (!bench.doneImplementationBenchmark())
    {
        ...
        for (int iter=0; iter < iters; ++iter)
            loop14_f77(N, x, a, b, y);
        bench.stop();

        bench.startOverhead();
        for (int iter=0; iter < iters; ++iter)
            loop14_f77overhead(N, x, a, b, y);
...
void F90Version(BenchmarkExt<int>& bench)
{
    ...
    while (!bench.doneImplementationBenchmark())
    {
        ...
        bench.start();
        for (int iter=0; iter < iters; ++iter)
            loop14_f90(N, x, a, b, y);
        bench.stop();

        bench.startOverhead();
        for (int iter=0; iter < iters; ++iter)
            loop14_f90overhead(N, x, a, b, y);
