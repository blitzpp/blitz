...
BZ_USING_NAMESPACE(blitz)
 ...
 #define loop15_f77 loop15_f77_
 #define loop15_f77overhead loop15_f77overhead_
 #define loop15_f90 loop15_f90_
 #define loop15_f90overhead loop15_f90overhead_
#elif defined(BZ_FORTRAN_SYMBOLS_WITH_DOUBLE_TRAILING_UNDERSCORES)
 #define loop15_f77 loop15_f77__
 #define loop15_f77overhead loop15_f77overhead__
 #define loop15_f90 loop15_f90__
 #define loop15_f90overhead loop15_f90overhead__
#elif defined(BZ_FORTRAN_SYMBOLS_CAPS)
 #define loop15_f77 LOOP15_F77
 #define loop15_f77overhead LOOP15_F77OVERHEAD
 #define loop15_f90 LOOP15_F90
 #define loop15_f90overhead LOOP15_F90OVERHEAD
#endif

extern "C" {
  void loop15_f77(const int& N, double* x, double* c, double* a, double* b);

  void loop15_f77overhead(const int& N, double* x, double* c, double* a, double* b);

  void loop15_f90(const int& N, double* x, double* c, double* a, double* b);

  void loop15_f90overhead(const int& N, double* x, double* c, double* a, double* b);
...
int main()
{
    ...
    BenchmarkExt<int> bench("loop15: $x=$c+$a*$b", numBenchmarks);
    ...
    bench.saveMatlabGraph("loop15.m");
...
void F77Version(BenchmarkExt<int>& bench)
{
    ...
    while (!bench.doneImplementationBenchmark())
    {
        ...
        for (int iter=0; iter < iters; ++iter)
            loop15_f77(N, x, c, a, b);
        bench.stop();

        bench.startOverhead();
        for (int iter=0; iter < iters; ++iter)
            loop15_f77overhead(N, x, c, a, b);
...
void F90Version(BenchmarkExt<int>& bench)
{
    ...
    while (!bench.doneImplementationBenchmark())
    {
        ...
        bench.start();
        for (int iter=0; iter < iters; ++iter)
            loop15_f90(N, x, c, a, b);
        bench.stop();

        bench.startOverhead();
        for (int iter=0; iter < iters; ++iter)
            loop15_f90overhead(N, x, c, a, b);
