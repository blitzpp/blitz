...
// floop22 generated by makeloops.py Wed Jun 29 17:04:04 2011
...
BZ_USING_NAMESPACE(std)
 ...
 #define floop22_f77 floop22_f77_
 #define floop22_f77overhead floop22_f77overhead_
 #define floop22_f90 floop22_f90_
 #define floop22_f90overhead floop22_f90overhead_
#elif defined(BZ_FORTRAN_SYMBOLS_WITH_DOUBLE_TRAILING_UNDERSCORES)
 #define floop22_f77 floop22_f77__
 #define floop22_f77overhead floop22_f77overhead__
 #define floop22_f90 floop22_f90__
 #define floop22_f90overhead floop22_f90overhead__
#elif defined(BZ_FORTRAN_SYMBOLS_CAPS)
 #define floop22_f77 FLOOP22_F77
 #define floop22_f77overhead FLOOP22_F77OVERHEAD
 #define floop22_f90 FLOOP22_F90
 #define floop22_f90overhead FLOOP22_F90OVERHEAD
#endif

extern "C" {
  void floop22_f77(const int& N, float* x, float* a, float* b, float* c, float* d);
  void floop22_f77overhead(const int& N, float* x, float* a, float* b, float* c, float* d);
  void floop22_f90(const int& N, float* x, float* a, float* b, float* c, float* d);
  void floop22_f90overhead(const int& N, float* x, float* a, float* b, float* c, float* d);
...
int main()
{
    ...
    BenchmarkExt<int> bench("floop22: $x = $x + $a*$b + $c*$d", numBenchmarks);
    ...
    bench.saveMatlabGraph("floop22.m");
    return 0;
}
  ...
  void ArrayVersion_index(BenchmarkExt<int>& bench)
{
    ...
    while (!bench.doneImplementationBenchmark())
    {
        ...
        for (long i=0; i < iters; ++i)
        {
            x = x(tensor::i) + a(tensor::i)*b(tensor::i) + c(tensor::i)*d(tensor::i);;
            sink();
        }
        bench.stop();
...
void F77Version(BenchmarkExt<int>& bench)
{
    ...
    while (!bench.doneImplementationBenchmark())
    {
        ...
        for (int iter=0; iter < iters; ++iter)
            floop22_f77(N, x, a, b, c, d);
        bench.stop();

        bench.startOverhead();
        for (int iter=0; iter < iters; ++iter)
            floop22_f77overhead(N, x, a, b, c, d);
...
void F90Version(BenchmarkExt<int>& bench)
{
    ...
    while (!bench.doneImplementationBenchmark())
    {
        ...
        bench.start();
        for (int iter=0; iter < iters; ++iter)
            floop22_f90(N, x, a, b, c, d);
        bench.stop();

        bench.startOverhead();
        for (int iter=0; iter < iters; ++iter)
            floop22_f90overhead(N, x, a, b, c, d);
