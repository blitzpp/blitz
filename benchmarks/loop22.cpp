...
BZ_USING_NAMESPACE(blitz)
 ...
 #define loop22_f77 loop22_f77_
 #define loop22_f77overhead loop22_f77overhead_
 #define loop22_f90 loop22_f90_
 #define loop22_f90overhead loop22_f90overhead_
#endif

#ifdef BZ_FORTRAN_SYMBOLS_WITH_DOUBLE_TRAILING_UNDERSCORES
 #define loop22_f77 loop22_f77__
 #define loop22_f77overhead loop22_f77overhead__
 #define loop22_f90 loop22_f90__
 #define loop22_f90overhead loop22_f90overhead__
#endif

#ifdef BZ_FORTRAN_SYMBOLS_CAPS
 #define loop22_f77 LOOP22_F77
 #define loop22_f77overhead LOOP22_F77OVERHEAD
 #define loop22_f90 LOOP22_F90
 #define loop22_f90overhead LOOP22_F90OVERHEAD
#endif

extern "C" {
  void loop22_f77(const int& N, double* x, double* a, double* b, double* c, double* d);

  void loop22_f77overhead(const int& N, double* x, double* a, double* b, double* c, double* d);

  void loop22_f90(const int& N, double* x, double* a, double* b, double* c, double* d);

  void loop22_f90overhead(const int& N, double* x, double* a, double* b, double* c, double* d);
...
int main()
{
    ...
    BenchmarkExt<int> bench("loop22: $x=$x+$a*$b+$c*$d", numBenchmarks);
    ...
    bench.saveMatlabGraph("loop22.m");
...
void F77Version(BenchmarkExt<int>& bench)
{
    ...
    while (!bench.doneImplementationBenchmark())
    {
        ...
        for (int iter=0; iter < iters; ++iter)
            loop22_f77(N, x, a, b, c, d);
        bench.stop();

        bench.startOverhead();
        for (int iter=0; iter < iters; ++iter)
            loop22_f77overhead(N, x, a, b, c, d);
...
void F90Version(BenchmarkExt<int>& bench)
{
    ...
    while (!bench.doneImplementationBenchmark())
    {
        ...
        bench.start();
        for (int iter=0; iter < iters; ++iter)
            loop22_f90(N, x, a, b, c, d);
        bench.stop();

        bench.startOverhead();
        for (int iter=0; iter < iters; ++iter)
            loop22_f90overhead(N, x, a, b, c, d);
