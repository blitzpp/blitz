...
// floop11 generated by makeloops.py Thu Jun 30 16:44:56 2011
...
BZ_USING_NAMESPACE(std)
 ...
 #define floop11_f77 floop11_f77_
 #define floop11_f77overhead floop11_f77overhead_
 #define floop11_f90 floop11_f90_
 #define floop11_f90overhead floop11_f90overhead_
#elif defined(BZ_FORTRAN_SYMBOLS_WITH_DOUBLE_TRAILING_UNDERSCORES)
 #define floop11_f77 floop11_f77__
 #define floop11_f77overhead floop11_f77overhead__
 #define floop11_f90 floop11_f90__
 #define floop11_f90overhead floop11_f90overhead__
#elif defined(BZ_FORTRAN_SYMBOLS_CAPS)
 #define floop11_f77 FLOOP11_F77
 #define floop11_f77overhead FLOOP11_F77OVERHEAD
 #define floop11_f90 FLOOP11_F90
 #define floop11_f90overhead FLOOP11_F90OVERHEAD
#endif

extern "C" {
  void floop11_f77(const int& N, float* x, float* a, float* b, float* c, float* d);
  void floop11_f77overhead(const int& N, float* x, float* a, float* b, float* c, float* d);
  void floop11_f90(const int& N, float* x, float* a, float* b, float* c, float* d);
  void floop11_f90overhead(const int& N, float* x, float* a, float* b, float* c, float* d);
...
int main()
{
    ...
    BenchmarkExt<int> bench("floop11: $x = $a+$b+$c+$d", numBenchmarks);
    ...
    bench.saveMatlabGraph("floop11.m");
    return 0;
}
...
void F77Version(BenchmarkExt<int>& bench)
{
    ...
    while (!bench.doneImplementationBenchmark())
    {
        ...
        for (int iter=0; iter < iters; ++iter)
            floop11_f77(N, x, a, b, c, d);
        bench.stop();

        bench.startOverhead();
        for (int iter=0; iter < iters; ++iter)
            floop11_f77overhead(N, x, a, b, c, d);
...
void F90Version(BenchmarkExt<int>& bench)
{
    ...
    while (!bench.doneImplementationBenchmark())
    {
        ...
        bench.start();
        for (int iter=0; iter < iters; ++iter)
            floop11_f90(N, x, a, b, c, d);
        bench.stop();

        bench.startOverhead();
        for (int iter=0; iter < iters; ++iter)
            floop11_f90overhead(N, x, a, b, c, d);
